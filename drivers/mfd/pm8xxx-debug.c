...
static int debug_shutdown_set(void *data, u64 val)
{
	int off = *(int *) data;
 ...
 DEFINE_SIMPLE_ATTRIBUTE(debug_shutdown_fops, NULL, debug_shutdown_set, "%lld\n");
...
static int __devinit pm8xxx_debug_probe(struct platform_device *pdev)
{
	...
	if (!strncmp(name, "pm8821", strlen("pm8821"))) {
		temp = debugfs_create_file("shutdown", S_IRUSR | S_IWUSR, dir, debugdev,
				   &debug_shutdown_fops);
		if (temp == NULL || IS_ERR(temp)) {
			pr_err("create debug_shutdown_fops failed: rc=%ld\n", PTR_ERR(temp));
		}
	}
