...
struct lut_reg_pair*g_lut_cache = NULL;
size_t g_lut_cache_size = 0;

static bool is_lvds_panel(void);

static bool is_lvds_panel(void)
{	return machine_is_apq8064_leopardcat(); }
...
u64 mdp_max_bw = 2480000000UL; // leopardcat 3000000000UL, see apq8064_display.c
u32 mdp_bw_ab_factor = MDP4_BW_AB_DEFAULT_FACTOR; // leopardcat 200
u32 mdp_bw_ib_factor = MDP4_BW_IB_DEFAULT_FACTOR; // leopardcat 250
...
int mdp_bus_scale_update_request(u64 ab_p0, u64 ib_p0, u64 ab_p1, u64 ib_p1)
{
	...
	if (!machine_is_apq8064_leopardcat()) {
		mdp_bus_usecases[bus_index].vectors[0].ab = mdp_max_bw;//min(ab_p0, mdp_max_bw);
		ib_p0 = max(ib_p0, ab_p0);
		mdp_bus_usecases[bus_index].vectors[0].ib = mdp_max_bw;//min(ib_p0, mdp_max_bw);

		mdp_bus_usecases[bus_index].vectors[1].ab = mdp_max_bw;//min(ab_p1, mdp_max_bw);
		ib_p1 = max(ib_p1, ab_p1);
		mdp_bus_usecases[bus_index].vectors[1].ib = mdp_max_bw;//min(ib_p1, mdp_max_bw);
	} else {
		mdp_bus_usecases[bus_index].vectors[0].ab = max(mdp_max_bw, ab_p0);
		ib_p0 = max(ib_p0, ab_p0);
		mdp_bus_usecases[bus_index].vectors[0].ib = max(mdp_max_bw, ib_p0);

		mdp_bus_usecases[bus_index].vectors[1].ab = max(mdp_max_bw, ab_p1);
		ib_p1 = max(ib_p1, ab_p1);
		mdp_bus_usecases[bus_index].vectors[1].ib = max(mdp_max_bw, ib_p1);
	}
		...
		if (mdp_pdata->cont_splash_enabled &&
			...
			mdp_pdata->splash_screen_addr =
				...
				if (alloc_boot_logo_copy_buffer
					...
					pr_info("BUFFER ALLOC Sucessfully done for SPLASH\n");
