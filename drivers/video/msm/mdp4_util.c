...
extern struct lut_reg_pair*g_lut_cache;
extern size_t g_lut_cache_size;
...
static int update_ar_gc_lut(uint32_t *offset, struct mdp_pgc_lut_data *lut_data)
{
	...
	size = lut_data->num_r_stages
		+ lut_data->num_g_stages
		+ lut_data->num_b_stages;
	// Every segment takes two registers and the enable register needs one
	// more extra slot.
	size = size * 2 + 1;
	if (g_lut_cache_size != size) {
		kfree(g_lut_cache);
		g_lut_cache_size = size;
		g_lut_cache = kmalloc(sizeof(struct lut_reg_pair) * g_lut_cache_size, GFP_KERNEL);
	}
			...
			outpdw(c0_params_offset+count,
				((0x7fff & lut_data->g_data[count].slope)
					...
					& lut_data->g_data[count].offset)
						<< 16)));
			g_lut_cache[i++].reg = c0_offset+count;
			g_lut_cache[i++].reg = c0_params_offset+count;
		} else
			...
			outpdw(c1_params_offset+count,
				((0x7fff & lut_data->b_data[count].slope)
					...
					& lut_data->b_data[count].offset)
						<< 16)));
			g_lut_cache[i++].reg = c1_offset+count;
			g_lut_cache[i++].reg = c1_params_offset+count;
		} else
		...
		if (count < lut_data->num_r_stages) {
			...
			outpdw(c2_params_offset+count,
				...
				& lut_data->r_data[count].offset)
					<< 16)));
			g_lut_cache[i++].reg = c2_offset+count;
			g_lut_cache[i++].reg = c2_params_offset+count;
		} else
			outpdw(c2_offset+count, 0);
	}
	size = i;
	for (i = 0; i < size; ++i)
		g_lut_cache[i].val = inpdw(g_lut_cache[i].reg);
