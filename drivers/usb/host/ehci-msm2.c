...
static int msm_ehci_ldo_init(struct msm_hcd *mhcd, int init)
{
	...
	if (hw_id >= 2) {// TF303K 0:SR 1:ER 2:PR 3:Reserved
		dev_info(mhcd->dev, "get HSUSB_1p8_l29");
		mhcd->hsusb_1p8 = devm_regulator_get(mhcd->dev, "HSUSB_1p8_l29");
	} else {
		dev_info(mhcd->dev, "get HSUSB_1p8");
...
static int usb3_phy_init_seq[] = {
	0x5D, 0x81, /* update DC voltage level */
//	0x24, 0x82, /* set pre-emphasis and rise/fall time */
	-1
};

static void msm_ulpi_init(struct msm_hcd *mhcd)
{
	//struct msm_otg_platform_data *pdata = motg->pdata;
	int *seq = usb3_phy_init_seq;

	//if (!seq)
	//	return;

	while (seq[0] >= 0) {
		printk(KERN_INFO "%s:ulpi: write 0x%x\n",
				__func__,msm_ulpi_read(mhcd, seq[1]));
		printk(KERN_INFO "%s:ulpi: write 0x%02x to 0x%02x\n",
				__func__,seq[0], seq[1]);
