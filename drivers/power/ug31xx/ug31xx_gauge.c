...
#define UG31XX_CALI_BO_LOOP_EOC         (10)
...
extern int smb345_chg_full_status(void);
extern int smb345_INOK;
...
#define UG31XX_IOCTL_ALGORITHM_READ_COS       _IOWR(UG31XX_IOC_MAGIC, 38, unsigned char *)
#define UG31XX_IOCTL_ALGORITHM_WRITE_COS      _IOWR(UG31XX_IOC_MAGIC, 39, unsigned char *)
...
static unsigned char op_options = LKM_OPTIONS_DEBUG_INFO;
...
static bool is_charging_full(void)
{
     ...
     (cur_cable_status != UG31XX_NO_CABLE))
  {
	...
	if(smb345_chg_full_status())
	{
		GAUGE_notice("[%s]: charger full reached.\n", __func__);
		return (true);
	}
	return (false);
}
...
static bool external_power_source_present(void)
{
	return (smb345_INOK ? false : true);
}
...
static long ug31xx_misc_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
  ...
  switch (UG31XX_IOCTL_CMD(cmd))
  {
    ...
    case (UG31XX_IOCTL_CMD(UG31XX_IOCTL_ALGORITHM_VERSION)):
      ...
      schedule_delayed_work(&ug31->cos_check_work,  msecs_to_jiffies(UG31XX_CALI_BO_FACTORY_DELAY+1));
#endif ///< for FEATRUE_K_BOARD_OFFSET
      break;
    ...
    case (UG31XX_IOCTL_CMD(UG31XX_IOCTL_ALGORITHM_MODE_COS)):
      UG31_LOGI("[%s] cmd -> UG31XX_IOCTL_ALGORITHM_MODE_COS\n", __func__);
      cos_mode = true;
      break;
      
    case (UG31XX_IOCTL_CMD(UG31XX_IOCTL_ALGORITHM_READ_COS)):
    ...
    case (UG31XX_IOCTL_CMD(UG31XX_IOCTL_ALGORITHM_WRITE_COS)):
...
{
        return cur_cable_status;
}
EXPORT_SYMBOL(get_cable_status);
...
static bool sw_charging_toggle = false;

static bool get_sw_charging_toggle(void)
{
  return (sw_charging_toggle);
}

static void start_charging(void)
{
  sw_charging_toggle = true;
  smb345_charger_enable(true);
}

static void stop_charging(void)
{
  sw_charging_toggle = false;
				      ...
				      const struct i2c_device_id *id)
{
  ...
  INIT_DELAYED_WORK(&ug31->curr_check_work, curr_check_work_func);
	...
	probe_with_cable = external_power_source_present();
	sw_charging_toggle = is_charging();
