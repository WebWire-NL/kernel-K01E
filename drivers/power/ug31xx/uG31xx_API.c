...
int lkm_initial(char *ggb, unsigned char cable)
{
  ...
  /// [AT-PM] : Force to 100% ; 07/09/2014
  if((ug31xx->batteryInfo.RSOC > 98) &&
     (ug31xx->capData.predictRsoc > 98))
  {
    upiGG_SetCapacity(lkm_gauge, 100);
    UG31_LOGE("[%s]: Force to 100 because RSOC = %d > 98 and Predict RSOC = %d > 98\n", __func__,
              ug31xx->batteryInfo.RSOC, ug31xx->capData.predictRsoc);
  }
