...
retry_send_app_op:
	...
	if (!retries) {
	    printk(KERN_ERR "%s: send_app_op_cond() fail (err = %d)\n"
		, mmc_hostname(host), err);

	    if (!strcmp("mmc1", mmc_hostname(host))
			&& !gpio_get_value(msm_host->plat->status_gpio)) {
		printk(KERN_ERR "%s: card still present. retry!\n"
			, mmc_hostname(host));
		goto retry_send_app_op;
	    } else
		return err;
	}
