...
static struct msm_camera_i2c_conf_array ov5693_confs[] = {
	...
	{&ov5693_video_90fps_settings[0],
	ARRAY_SIZE(ov5693_video_90fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
	{&ov5693_video_120fps_settings[0],
	ARRAY_SIZE(ov5693_video_120fps_settings), 0, MSM_CAMERA_I2C_BYTE_DATA},
};
...
static struct msm_camera_csi2_params *ov5693_csi_params_array[] = {
	...
	&ov5693_csi_params, /* 60 fps */
	&ov5693_csi_params, /* 90 fps */
	&ov5693_csi_params, /* 120 fps */
};
#endif
...
#define	OV5693_PROC_OTP_VALUE	"driver/otp_value"
...
void create_ov5693_proc_file(void)
{
	...
	if (create_proc_read_entry(OV5693_PROC_OTP_VALUE, 0666, NULL,
			ov5693_proc_read_otp_value, NULL) == NULL) {
		pr_err("[Camera]proc file otp_value create failed!\n");
	}
}
...
static int i2c_write_af_data(struct file *file, char __user *buffer,
	size_t count, loff_t *ppos)
{
	int len = 0;
	char *bp = debugTxtBuf;
	...
	fp = filp_open("/sdcard/set_af_position.txt",
	...
	if (!IS_ERR_OR_NULL(fp)) {
		...
		if (fp->f_op != NULL && fp->f_op->read != NULL) {
				pr_info("file set_af_position.txt start to read!\n");
				while (fp->f_op && count_temp != 1) {
					count_temp = fp->f_op->read(fp, buf,
						sizeof(buf), &fp->f_pos);
					sscanf(buf, "0x%x 0x%x 0x%x\n",
						&temp[0], &temp[1], &temp[2]);
					pr_info("temp: 0x%x 0x%x 0x%x",
						temp[0], temp[1], temp[2]);
					if (temp[0] == 0x10) {
						data[0] = (u8) (temp[1] >> 8);
						data[1] = (u8) (temp[1] & 0xff);
						data[2] = (u8) (temp[2] & 0xff);
						msg.addr = temp[0];
						msg.flags = 0;
						msg.len = 3;
						msg.buf = data;
					} else {
						data[0] = (u8) (temp[1] & 0xff);
						data[1] = (u8) (temp[2] & 0xff);
						msg.addr = temp[0];
						msg.flags = 0;
						msg.len = 2;
						msg.buf = data;
		...
		set_fs(old_fs);
		filp_close(fp, NULL);
		len = snprintf(bp, DBG_TXT_BUF_SIZE, "Success read file\n");
	} else {
		pr_info("filp_open fail fp:%p\n", fp);
		len = snprintf(bp, DBG_TXT_BUF_SIZE, "Fail to read file\n");
	}

	if (copy_to_user(buffer, debugTxtBuf, len))
		return -EFAULT;
...
static const struct file_operations write_af_data = {
	.open = i2c_set_open,
	.read = i2c_write_af_data,
};
...
static int __init qualcomm_i2c_debuginit(void)
{
	...
	(void) debugfs_create_file("write_af_data", S_IRUGO | S_IWUSR,
			dent, NULL, &write_af_data);
