	...
	uint16_t res)
{
	...
	uint32_t fll = (s_ctrl->msm_sensor_reg->
		...
		s_ctrl->fps_divider) / Q10;
	struct msm_camera_i2c_reg_conf dim_settings_0[] = {
	...
	struct msm_camera_i2c_reg_conf dim_settings_1[] = {
		{s_ctrl->sensor_output_reg_addr->x_output,
			s_ctrl->msm_sensor_reg->
			output_settings[res].x_output-8},
		{s_ctrl->sensor_output_reg_addr->y_output,
			s_ctrl->msm_sensor_reg->
			output_settings[res].y_output-8},
		{s_ctrl->sensor_output_reg_addr->line_length_pclk,
			s_ctrl->msm_sensor_reg->
			output_settings[res].line_length_pclk},
		{s_ctrl->sensor_output_reg_addr->frame_length_lines,
	...
	if (s_ctrl->sensor_i2c_addr == 0x20) { //OV5693
		rc = msm_camera_i2c_write_tbl(s_ctrl->sensor_i2c_client, dim_settings_0,
			ARRAY_SIZE(dim_settings_0), MSM_CAMERA_I2C_WORD_DATA);
		return rc;
	}
	else if (s_ctrl->sensor_i2c_addr == 0x90) { //MI1040
		rc = msm_camera_i2c_write_tbl(s_ctrl->sensor_i2c_client, dim_settings_1,
			ARRAY_SIZE(dim_settings_1), MSM_CAMERA_I2C_WORD_DATA);
		return rc;
	}
	return 0;
}
