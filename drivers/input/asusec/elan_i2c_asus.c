				...
				u8 *val, u16 len)
{
	...
	do{
		...
		if (ret != 2)
			ELAN_ERR("wrong length ret: %d, msg_len: %d\n", ret, len);
		else	break;
		tries--;
...
int elan_i2c_initialize(struct i2c_client *client)
{
	...
	ELAN_INFO("elan_i2c_initialize start \n");
...
//	uint16_t x, y;
#if ELAN_TOUCHPAD_ONLY_LEFT_BUTTOM
	int left_button, right_button;
#elif ELAN_TOUCHEPAD_RIGHT_BUTTOM
	int finger_right = 0;
	u8 press_button;
#endif
	...
	*/
	...
	for (i = 0 ; i < ETP_MAX_FINGERS ; i++) {
		finger_on = (packet[3] >> (3 + i)) & 0x01;//check finger

		if (finger_on) {
			//finger_on_num ++;

			pos_x = ((finger_data[0] & 0xf0) << 4) | finger_data[1];
			pos_y = data->max_y - (((finger_data[0] & 0x0f) << 8) | finger_data[2]);

			area_x = (finger_data[3] & 0x0f) * data->width_x;
			area_y = (finger_data[3] >> 4) * data->width_y;
			pressure = finger_data[4];

#if ELAN_BUTTON_WITHOUT_TOUCH_POSITION
			if(pos_y >= 987 && pos_y <= 1480) {
#if ELAN_TOUCHPAD_ONLY_LEFT_BUTTOM
#elif ELAN_TOUCHEPAD_RIGHT_BUTTOM
				if(pos_x > (data->max_x/2)) finger_right = BTN_RIGHT;
	...
	//ELAN_INFO("PosX =%d PosY=%d\n",pos_x,pos_y);

#if ELAN_TOUCHPAD_ONLY_LEFT_BUTTOM
	left_button = (packet[3] & 0x01);
	if (left_button != data->left_button ) {
		if(left_button){
			ELAN_INFO("send left key event");
	...
	right_button = (packet[3] & 0x02) >> 1;
	if (right_button != data->right_button) {
		if(right_button){
			ELAN_INFO("send right key event");
...
#elif ELAN_TOUCHEPAD_RIGHT_BUTTOM
	//if(finger_on_num != 1) finger_right = 0;
				     ...
				     u8 *packet)
{
	...
	ELAN_INFO("elan_i2c_report_absolute\n");
	return;
}
...
int elan_i2c_check_packet(u8 *packet)
{
	...
	*/
	...
	//ELAN_INFO("elan check packet length=%x(%x), report_id=%x(%x)\n",length, ETP_REPORT_LENGTH,report_id,ETP_REPORT_ID);

	if (length == ETP_REPORT_LENGTH && report_id == ETP_REPORT_ID){
		//ELAN_INFO("elan absolute mode packet\n");
		return 0;
	}else if (length == 7 && report_id == 0x01){
		//ELAN_INFO("elan standard mode packet\n");
		return 1;
	}else{
		ELAN_INFO("elan packet error\n");
		return -1;
	}
}
