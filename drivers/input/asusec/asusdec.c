...
static unsigned int asusdec_dock_power =32;

#define ASUSDEC_APWAKE_GPIO	PM8921_GPIO_PM_TO_SYS(21)
#define ASUSDEC_ECREQ_GPIO	PM8921_GPIO_PM_TO_SYS(22)
static unsigned int asusdec_apwake_gpio = ASUSDEC_APWAKE_GPIO;//8921 PMIC_GPIO
static unsigned int asusdec_ecreq_gpio = ASUSDEC_ECREQ_GPIO;//8921 PMIC_GPIO
static unsigned int asusdec_dock_in_gpio = 44;
#if ASUSEC_LID
static unsigned int asusdec_hall_sensor_gpio = 58;
#endif
...
void ec_irq_disable(void)
{
	...
	//ASUSDEC_NOTICE("ec_irq_disable flag = %d\n", ec_chip->apwake_disabled);
...
void ec_irq_enable(void)
{
	unsigned long irqflags;
	spin_lock_irqsave(&ec_chip->irq_lock, irqflags);
	//ASUSDEC_NOTICE("ec_irq_enable flag = %d\n", ec_chip->apwake_disabled);
	if (ec_chip->apwake_disabled)
	{
...
unsigned char asusdec_crc8(unsigned char *ptr, unsigned char len)
{
	unsigned char i;
	unsigned char crc=0;
	while(len--!=0)
...
unsigned char asusdec_crc_add_addr(unsigned char *ptr, unsigned char len)
{
	unsigned char temp[len+3];
	memcpy(&temp[3], ptr, len);
	temp[0] = 0x36;//8bit write slave address
	temp[1] = 0x41;//flash command
	temp[2] = len;
	return(asusdec_crc8(temp, len+3));
}
...
#if ASUSDEC_CRC8
	//add packet err checking
	ec_chip->i2c_fu_data[count] = asusdec_crc_add_addr(ec_chip->i2c_fu_data, count);
...
static ssize_t asusdec_cmd_data_store(struct device *class,struct device_attribute *attr,const char *buf, size_t count)
{
	...
	int i=0, j=0, idx=0, ret=0, data_cnt;
	//int cmd;
	u8 cmd=0,i2c_type=0;
	...
	switch(i2c_type){
		...
		case 0x04:
			...
			break;
		case 0x05:
			ret = i2c_smbus_write_word_data(&dockram_client, cmd, (int)ec_chip->i2c_dm_data);
...
#if ASUSDEC_DEBUG
	ASUSDEC_NOTICE("asusdec_interrupt_handler IRQ=%d\n",irq);
//	ASUSDEC_INFO("irq = %d GPIO = %d , state = %d\n", irq, gpio, gpio_get_value(gpio));
	...
	if(irq==asusdec_dock_in_gpio_irq){
		...
		if (ec_chip->op_mode){
			ASUSDEC_INFO("into susdec_fw_update_work");
...
static int asusdec_kp_consumer_key_mapping(int x)
{
	switch (x){
		...
		case ASUSDEC_KEYPAD_SUSPEND:
			return KEY_SLEEP;
		default:
			ASUSDEC_NOTICE("No consumer mapping string\n");
			return -1;
	}
}
...
static int asusdec_kp_key_mapping(int x)
{
	switch (x){
		...
		case ASUSDEC_EUROPE_2:
			return KEY_102ND;
		default:
			ASUSDEC_NOTICE("No mapping string\n");
			return -1;
	}
}
...
static void asusdec_kb_report_work_function(struct work_struct *dat)
{
	...
	if(ec_chip->suspend_state){
		//ASUSDEC_NOTICE("jjt wake system\n");
...
static void asusdec_tp_report_work_function(struct work_struct *dat) {
    ...
    //ASUSDEC_NOTICE("asusdec_tp_report_work_function retval = %d", retval);
...
#if ASUSEC_LID
static void asusdec_hall_sensor_work_function(struct work_struct *dat)
{
	int value = 0;
...
static int asusdec_input_device_create(struct i2c_client *client){
	...
	ec_chip->indev->phys = "/dev/input/asuspec";//for AMAX
...
#if ASUSDEC_DEBUG
	ASUSDEC_NOTICE("dock in gpio state = %d\n",gpio_state);
#endif
	if (gpio_state){
...
//jj-		asusdec_tp_enable_function(0);
		//Shouchung end
		//Shouchung add for enable touchpad when dock-in
		ec_chip->tp_enable = 0;
		ec_chip->tp_ioctl_state= 1;
		//Shouchung end
		...
		/*Chris start*/
		if(asusdec_input_device_create(&kb_client)){
			goto fail_to_access_ec;
		}
		ASUSDEC_NOTICE("asusdec_input_device_create out\n");
		memset(&ec_chip->i2c_kb_data, 0, 32);
...
static void asusdec_touchpad_init_work_function(struct work_struct *dat)
{
	...
	if ((ret_val >= 0) || (ec_chip->i2c_dm_data[3] & ASUSDEC_HID_STATUS) || (1/* //jjt_t100_test_dock  !gpio_get_value(asusdec_dock_in_gpio)*/)){
		...
		if(!elan_i2c_initialize(&tp_client)){
			...
			ec_chip->tp_status= 1;
			if(ec_chip->tp_ioctl_state == 0)
				asusdec_tp_control(0);
		}else{
			ec_chip->tp_enable = 0;
...
//	int irq = gpio_to_irq(gpio);
	...
	mutex_lock(&ec_chip->lock);
        ...
        case UPDATE_BYTE_MODE:
            ...
            break;
        case UPDATE_BLOCK_MODE:
    	    ret = i2c_smbus_read_i2c_block_data(&dockram_client, 0x6a, 8/*32*/, ec_chip->i2c_fu_read_data);
    	    if (ret < 0)
    	    	ASUSDEC_ERR("fu work Fail to read data, status %02x\n", ret);
    	    for (i = 0; i < ec_chip->i2c_fu_read_data[0] + 2 ; i++){
    	    	BuffPush(ec_chip->i2c_fu_read_data[i]);//FIXME:only need push fa
    	    }
            ec_irq_enable();
            break;
...
#endif
	int ret_val = 0;
	ASUSDEC_INFO("show irq value --- gpio =%d irq = %d \n",gpio,irq);
...
static long asusdec_ioctl(struct file *flip,
	...
	switch (cmd) {
...
#if ASUSDEC_CRC8
				ASUSDEC_NOTICE("use crc8 function in ec driver\n");
#else
		...
		case ASUSDEC_TP_CONTROL:
		       ASUSDEC_NOTICE("ASUSDEC_TP_CONTROL: %ld\n",arg);
                     if ((ec_chip->op_mode == 0) && ec_chip->dock_in){
                         asusdec_tp_control(arg);
                         ec_chip->tp_ioctl_state = arg;
                         return err;
                        }
		       else
			    return -ENOTTY;
		...
		case ASUSDEC_FW_DUMMY:
			...
			case UPDATE_BLOCK_MODE:
				...
				i2c_smbus_write_i2c_block_data(&dockram_client, 0x41, 3, ec_chip->i2c_dm_data);
#if ASUSDEC_CRC8
				ASUSDEC_NOTICE("fu dummy use crc8 function in ec driver\n");
#else
				dockram_client.flags = I2C_CLIENT_PEC;
#endif
				msleep(500);
		...
		const struct i2c_device_id *id)
{
	int err = 0;
        ...
        static struct regulator  *ec_pwr;
	int rc = 0;

	ec_pwr = regulator_get(NULL, "ec_pwr");
	if (IS_ERR_OR_NULL(ec_pwr)) {
		ASUSDEC_ERR("could not get ec_pwr, rc = %ld\n",
			PTR_ERR(ec_pwr));
	}
	rc = regulator_set_voltage(ec_pwr, 3000000, 3000000);
	if (rc) {
		ASUSDEC_ERR("set_voltage ec failed, rc=%d\n", rc);
	}
	rc = regulator_enable(ec_pwr);
	if (rc) {
		ASUSDEC_ERR("enable ec pwr failed, rc=%d\n", rc);
	}

	ASUSDEC_NOTICE("get ec voltage: %d!!\n",regulator_get_voltage(ec_pwr));

	ASUSDEC_INFO("asusdec probe!!!\n");
	/*initial gpio*/

	//set irq for interrupt
	asusdec_apwake_gpio_irq = gpio_to_irq(asusdec_apwake_gpio);
	/*PRINT ALL GPIO SETTING*/
	ASUSDEC_NOTICE("GPIO asusdec_dock_in_gpio = %d\n", asusdec_dock_in_gpio);
	ASUSDEC_NOTICE("GPIO asusdec_apwake_gpio = %d\n", asusdec_apwake_gpio);
	ASUSDEC_NOTICE("GPIO asusdec_apwake_gpio_irq = %d\n", asusdec_apwake_gpio_irq);
	ASUSDEC_NOTICE("GPIO asusdec_ecreq_gpio = %d\n", asusdec_ecreq_gpio);
	ASUSDEC_NOTICE("GPIO asusdec_dock_power = %d\n", asusdec_dock_power);
...
//	ec_chip->kb_and_ps2_enable = 0;
	...
	asusdec_irq_hall_sensor(client);
        INIT_DELAYED_WORK(&ec_chip->asusdec_hall_sensor_work, asusdec_hall_sensor_work_function);
#endif
...
static int asusdec_resume(struct i2c_client *client){
	int ret_val;
#if ASUSEC_LID
	asusdec_hall_sensor_work_function(NULL);
#endif
	//[Brook- Fix bug 295323 + System can't wakeup with dcoking when insert/removed AC]>>
	...
	if (ec_chip->suspend_state && ec_chip->dock_in ) {
		...
		ec_chip->touchpad_member = -1;
	        ec_chip->tp_enable = 0;
		ret_val = asusdec_susb_control(1);
		if (ret_val < 0)
