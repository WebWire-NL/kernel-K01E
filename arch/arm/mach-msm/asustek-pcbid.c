...
static unsigned int tp_ic_type_pcbid[] = {13};
...
struct pcbid_maps {
	...
	{"TP_IC_TYPE", tp_ic_type_pcbid, ARRAY_SIZE(tp_ic_type_pcbid)},
};
...
{
	tp_ic_type ret = TP_IC_TYPE_INVALID;

	if (machine_is_apq8064_leopardcat()) {
		ret = get_pcbid_type("TP_IC_TYPE");

		if (debug_mask & DEBUG_VERBOSE)
			pr_info("%s: %d\n", __func__, ret);

		if ((ret == -ENODEV) || (ret >= TP_IC_TYPE_MAX))
			ret = TP_IC_TYPE_INVALID;
	}
...
static int __init pcbid_driver_probe(struct platform_device *pdev)
{
	...
	if (machine_is_apq8064_leopardcat() &&
		(asustek_pcbid != PCBID_VALUE_INVALID)) {
		pdata_res = pdata->resource_leopardcat;
		nr_pdata_res = pdata->nr_resource_leopardcat;
		for (j = 0; j < nr_pdata_res; j++) {
			...
			ret = gpio_request((&pdata_res[j])->start,
						(&pdata_res[j])->name);
			if (ret) {
				/* indicate invalid value when error happens */
				pr_err("ASUSTek: Failed to request gpio%d for %s\n",
							(&pdata_res[j])->start,
							(&pdata_res[j])->name);
				asustek_pcbid = PCBID_VALUE_INVALID;
			...
			gpio_direction_input((&pdata_res[j])->start);
			asustek_pcbid |= gpio_get_value((&pdata_res[j])->start)
						<< (pdev->num_resources + j);
		}
	}
...
arch_initcall(asustek_pcbid_init);
