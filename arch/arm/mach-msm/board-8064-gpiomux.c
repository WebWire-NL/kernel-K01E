...
static struct gpiomux_setting ec_setting = {
	.func = GPIOMUX_FUNC_GPIO,
	.drv = GPIOMUX_DRV_2MA,
	.pull = GPIOMUX_PULL_NONE,
...
static struct msm_gpiomux_config asustek_ec_configs[] __initdata = {
	{
		.gpio = 44,	//EC dock_in
		.settings = {
			[GPIOMUX_ACTIVE]    = &ec_setting,
			[GPIOMUX_SUSPENDED] = &ec_setting,
		},
	},
};
...
static struct msm_gpiomux_config msm8064_macaque_ts_gpio_config[] __initdata = {
	{
		.gpio = 6,	/* tp IRQ*/
		.settings = {
...
void __init apq8064_init_gpiomux(void)
{
	...
	if (!machine_is_apq8064_leopardcat()) {
		...
		printk("leopardcat: skip apq8064_ext_regulator_configs.\n");
	}
	...
	if (machine_is_apq8064_mtp() || machine_is_apq8064_macaque())
		msm_gpiomux_install(asustek_ec_configs,
			ARRAY_SIZE(asustek_ec_configs));

	msm_gpiomux_install(apq8064_headphone_configs,
			ARRAY_SIZE(apq8064_headphone_configs));

	msm_gpiomux_install(msm8064_macaque_ts_gpio_config,
			ARRAY_SIZE(msm8064_macaque_ts_gpio_config));
}
