...
static struct pm8xxx_gpio_init pm8921_gpios[] __initdata = {
	...
	PM8921_GPIO_INPUT(21, PM_GPIO_PULL_UP_30),     /* EC APWAKE*/
	PM8921_GPIO_INPUT(22, PM_GPIO_PULL_UP_30),     /* EC REQ*/
};

static struct pm8xxx_gpio_init leopardcat_pm8921_gpios[] __initdata = {
	PM8921_GPIO_OUTPUT(14, 1, HIGH),	/* HDMI Mux Selector */
	// PM8921_GPIO_OUTPUT(23, 1, HIGH),	/* EN_VDD_BL */
	// PM8921_GPIO_OUTPUT_BUFCONF(25, 0, LOW, CMOS), /* DISP_RESET_N */
	PM8921_GPIO_OUTPUT_FUNC(26, 0, PM_GPIO_FUNC_2), /* Bl: Off, PWM mode */
...
static struct pm8xxx_gpio_init pm8921_asustek_kp_gpios[] __initdata = {
	PM8921_GPIO_INPUT(4, PM_GPIO_PULL_UP_30),	/* VOL_UP */
	PM8921_GPIO_INPUT(38, PM_GPIO_PULL_UP_30),	/* VOL_DOWN */
};

static struct pm8xxx_gpio_init pm8921_asustek_camera_id_gpios[] __initdata = {
	PM8921_GPIO_INPUT(6, PM_GPIO_PULL_NO),		/* MAIN_CAM_ID0 */
};
...
void __init apq8064_pm8xxx_gpio_mpp_init(void)
{
	...
	if (socinfo_get_pmic_model() != PMIC_MODEL_PM8917) {
		if (machine_is_apq8064_leopardcat()) {
			printk("Leopardcat: configure gpio.\n");
			apq8064_configure_gpios(leopardcat_pm8921_gpios,
					ARRAY_SIZE(leopardcat_pm8921_gpios));
		} else {
	...
	if (machine_is_apq8064_mtp() || machine_is_apq8064_macaque() ||
		...
		apq8064_configure_gpios(pm8921_asustek_kp_gpios,
					ARRAY_SIZE(pm8921_asustek_kp_gpios));
	...
	if (machine_is_apq8064_leopardcat()) {
		/*
		 * gpio pin for CAM_ID at HW_REV_A stage is open so that
		 * internal pull up is given for initial state.
		 */
		if (asustek_get_hw_rev() == HW_REV_A)
			pm8921_asustek_camera_id_gpios[0].config.pull =
				PM_GPIO_PULL_UP_30;

		apq8064_configure_gpios(pm8921_asustek_camera_id_gpios,
				ARRAY_SIZE(pm8921_asustek_camera_id_gpios));
	}
	...
	if (!machine_is_apq8064_leopardcat()) {
		...
		printk("leopardcat: skip pm8xxx_mpps.\n");
	}
}

static void apq8064_pm8921_macaque_pwm_init(void) {
	int ret;
	printk("%s+\n", __func__);
...
static struct pm8xxx_led_platform_data apq8064_pm8921_leds_pdata = {
		...
		.pwm_init = apq8064_pm8921_macaque_pwm_init,
};
