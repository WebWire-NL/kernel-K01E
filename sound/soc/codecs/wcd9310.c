...
static struct pm_qos_request *c6_restrict_qos;
static bool idle_enable;
...
static int tabla_get_qos_func(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	printk("%s: ++ %d\n", __func__, idle_enable);
	ucontrol->value.enumerated.item[0] = (int)idle_enable;

	return 0;
}

static int tabla_put_qos_func(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	printk("%s: ++ %d\n", __func__, ucontrol->value.enumerated.item[0]);
	if (c6_restrict_qos) {
		if (ucontrol->value.enumerated.item[0] == 0) {
			pm_qos_update_request(c6_restrict_qos, msm_cpuidle_get_deep_idle_latency());
			idle_enable = false;
		} else {
			pm_qos_update_request(c6_restrict_qos, PM_QOS_DEFAULT_VALUE);
			idle_enable = true;
		}
	}
...
static const char *const tabla_qos_func_text[] = {"IDLE_DISABLE", "IDLE_ENABLE"};
static const struct soc_enum tabla_qos_func_enum =
	SOC_ENUM_SINGLE_EXT(2, tabla_qos_func_text);
...
static const struct snd_kcontrol_new tabla_snd_controls[] = {
	...
	SOC_ENUM_EXT("ANC Function", tabla_anc_func_enum, tabla_get_anc_func,
		tabla_put_anc_func),
	SOC_ENUM_EXT("CState Control", tabla_qos_func_enum, tabla_get_qos_func,
		tabla_put_qos_func),
...
#endif
	idle_enable = true;
	c6_restrict_qos = kzalloc(sizeof(struct pm_qos_request), GFP_KERNEL);
	pm_qos_add_request(c6_restrict_qos, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
...
#endif
	pm_qos_remove_request(c6_restrict_qos);
	kfree(c6_restrict_qos);
